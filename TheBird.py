...
# --- VENTANA EMERGENTE MAESTRA (CON SOPORTE DE MÚLTIPLE SELECCIÓN) ---
...
class CreacionCompletaModal(discord.ui.Modal, title="Crear Pregunta y Opciones"):
    ...
    multiple_txt = discord.ui.TextInput(label="¿Varias respuestas a la vez? (si / no)", placeholder="no", required=True, default="no", max_length=3)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        is_multiple = self.multiple_txt.value.strip().lower() in ["si", "sí", "true", "1"]

        if id_c not in datos:
            datos[id_c] = {
                "titulo": id_c.replace("_", " ").title(),
        ...
        multi_msg = "Sí (Varias respuestas)" if is_multiple else "No (Una sola respuesta)"
        await interaction.response.send_message(
            f"✅ ¡Pregunta `{clave_p}` creada con éxito!\n"
            f"• Múltiple selección: **{multi_msg}**\n"
            f"• Opciones añadidas: **{len(opciones_lista)}**",
            ephemeral=True
        )

# --- COMANDOS SLASH (/) DE GESTIÓN Y ELIMINACIÓN ---

@bot.tree.command(name="crear_pregunta", description="Abre una ventana para crear un cuestionario, pregunta y opciones.")
...
@bot.tree.command(name="cambiar_titulo", description="Cambia el título visible de un cuestionario.")
@app_commands.describe(id_cuestionario="ID del cuestionario", nuevo_titulo="Nuevo título oficial")
@app_commands.checks.has_permissions(administrator=True)
async def cambiar_titulo(interaction: discord.Interaction, id_cuestionario: str, nuevo_titulo: str):
    datos = cargar_datos()
    if id_cuestionario not in datos:
        ...
        return
    datos[id_cuestionario]["titulo"] = nuevo_titulo
    guardar_datos(datos)
    await interaction.response.send_message(f"✅ Título del cuestionario **{id_cuestionario}** cambiado a: *{nuevo_titulo}*.", ephemeral=True)
